...
export default function App() {
  ...
  return (
    <div className={
      ...
      <header className={
        "w-full max-w-5xl flex justify-between items-center mb-6 md:mb-8 " +
        ...
        <div className="flex flex-col">
          ...
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-xl md:text-2xl font-black tracking-wider text-transparent bg-clip-text bg-gradient-to-r from-pink-400 to-purple-400">
      ...
      {mode === 'receiver' && (
        <div className={
          "w-full flex flex-col items-center gap-4 md:gap-6 z-10 relative " +
          ...
          <div className="w-full flex flex-col md:flex-row items-stretch justify-center gap-4 md:gap-8">
            ...
            {!isConnected && (
              <div className={
                "w-full md:w-1/3 bg-white/[0.03] backdrop-blur-xl border border-white/10 rounded-[24px] md:rounded-[32px] " +
                "p-6 md:p-8 text-center flex flex-col items-center justify-center shrink-0 shadow-2xl"
              }>
                <h3 className="text-xl md:text-2xl font-bold mb-1 md:mb-2 tracking-tight">Pair Device</h3>
                <p className="text-xs md:text-sm text-white/50 mb-6 md:mb-8 font-light">Point your mobile lens at this matrix</p>
                
                <div className="bg-white p-4 md:p-5 rounded-[20px] md:rounded-[24px] mb-6 md:mb-8 flex justify-center items-center min-h-[160px] md:min-h-[200px] min-w-[160px] md:min-w-[200px] shadow-[0_0_30px_rgba(255,255,255,0.1)]">
                  {peerId ? (
                    <QRCodeSVG value={peerId} size={180} style={{ width: '100%', height: 'auto', maxWidth: '180px' }} />
                  ) : (
                    <div className="w-32 md:w-40 h-32 md:h-40 flex flex-col items-center justify-center text-black/40 gap-2 md:gap-3">
                      <RefreshCw className="animate-spin text-purple-600" size={24} />
                      <span className="text-[10px] md:text-xs font-bold uppercase tracking-wider">Generating Key</span>
                    </div>
                  )}
                </div>

                <div className="w-full bg-black/50 rounded-xl md:rounded-2xl p-3 md:p-4 border border-white/10 max-w-full overflow-hidden shadow-inner">
                  <code className="text-[10px] md:text-xs text-purple-300 break-all block font-mono">{peerId || 'Awaiting secure token...'}</code>
                </div>
              </div>
            )}

            <div className={
              "bg-black rounded-[24px] md:rounded-[32px] border border-white/10 relative overflow-hidden flex items-center justify-center shadow-[0_0_50px_rgba(0,0,0,0.5)] " +
              (isConnected ? 'w-full h-[calc(100vh-140px)]' : 'w-full md:w-2/3 min-h-[100px] md:min-h-[500px]')
            }>
              <video 
                ref={remoteVideoRef} 
                autoPlay 
              ...
              {!isConnected && (
                <div className="absolute inset-0 flex flex-row md:flex-col items-center justify-center bg-black/40 backdrop-blur-lg gap-4 md:gap-6 text-left md:text-center p-4 md:p-8">
                  <div className="p-3 md:p-6 bg-purple-500/10 rounded-full border border-purple-500/20 text-purple-400 animate-pulse shadow-[0_0_30px_rgba(168,85,247,0.2)] shrink-0">
                  ...
                  <div>
                    ...
                    <p className="hidden md:block text-base text-white/50 max-w-md leading-relaxed font-light mt-2">
