...
export default function App() {
  ...
  useEffect(() => {
    ...
    // --- FIX: Forced Native Permissions & Auto-Start ---
    if (mode === 'camera') {
      setTimeout(() => {
        ...
        // 1. Force the OS to prompt the user for permission instantly
        Html5Qrcode.getCameras().then(devices => {
          if (devices && devices.length) {
            
            // 2. Automatically hunt down the rear/back camera
            ...
            // 3. Start scanning instantly without user clicks
            html5QrCode.start(
              selectedCameraId,
              {
                fps: 10,
              ...
              (decodedText) => {
                // Success! Stop scanner and connect
              ...
              (errorMessage) => {
                // Background frame errors (ignore)
              }
            ).then(() => {
          ...
          // If the user clicks "Deny" on the popup, this catches it
          console.error("Permission denied:", err);
          setStatus("Permission Denied. Please allow camera access.");
        });

      }, 150); // Tiny wait just to let the React UI render the box first
    }
  ...
  const executeManualConnect = () => {
    if (scannerInstanceRef.current && scannerInstanceRef.current.isScanning) {
      scannerInstanceRef.current.stop().then(() => {
        handleConnectToPC(remoteId);
      }).catch(() => handleConnectToPC(remoteId));
    } else {
      handleConnectToPC(remoteId);
    }
  };
        ...
        <div className="w-full max-w-sm flex flex-col items-center gap-4">
          ...
          {!isConnected ? (
            <div className="w-full bg-white/[0.02] border border-white/10 rounded-2xl p-4 text-center flex flex-col gap-3">
              ...
              <div id="reader" className="overflow-hidden rounded-xl bg-black/50 border border-white/5 text-white max-w-full min-h-[250px] flex items-center justify-center relative">
